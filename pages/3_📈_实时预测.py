...
def real_api(well_name_us,well_name_cn,token):
    data, ok = GetoneWellInfo(well_name_us, well_name_cn, token)
    return  data,ok
...
def jiexi_url(url):
    ...
    well_name = path.split('/')[-2]

    # 提取访问令牌
    query_params = parse_qs(parsed_url.query)
    access_token = query_params.get('access_token', [''])[0]
    return well_name,access_token
...
def GetoneWellInfo(well_name_us: str, well_name_cn: str, token: str):
    ...
    url1 = "http://10.76.204.222:10778/dataservices/services/%s/VIEW?$filter=DEPTH ge %s and DEPTH le %s&access_token=%s" % (
    ...
    for item in data1['value']:
        ...
        SPP.append(item['SPP'])
        DRITIME.append(item['DRITIME'])
        if item['VOP'] :
            VOP.append(item['VOP'])
        else:
            try:
                VOP.append(1/item['DRITIME'])
            except:
                VOP.append(0.0)
...
def main():
    ...
    well_name = col2.text_input("请输入要监控的井名：", "Well1")
    url_real = col3.text_input("请输入监控API接口(url)：", "")
    star_depth = col4.number_input("监测起始井深：",0,1000)
    current_depth=star_depth
    if st.session_state['block_seting']==1:
        ...
        if co11.button("开始监控"):
            well_name_us, token = jiexi_url(url_real)
            if star_depth < 2 :
                GetStratDepth(well_name_us, token)
                get_max_well_depth(conn, table_name, well_name)
            else:
                st.write(star_depth)
            ...
            while True:
                ...
                data, ok = real_api(well_name_us, well_name, token)
                print(data)
                if ok=='':
 ...
 # --------------------------------------------提前预测-------------------------------------------------
                    if len(data)>80:
                        ...
                        def predict_properties(data, column_names):
                            ...
                            # data = data.sort_values(by='井深',ascending=True)
                            last_80_data = data[-80:]
                            print(last_80_data)

                            feature_names = ['大勾载荷', '泵压', '机械钻速', '钻压', '转速', '排量', '密度', '粘度']
                            last_80_data= last_80_data[feature_names]
                            last_80_data=last_80_data.fillna(0)
